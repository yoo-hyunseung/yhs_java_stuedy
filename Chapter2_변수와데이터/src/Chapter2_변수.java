...
public class Chapter2_변수 {
	 ...
	 *  	컴파일(javac) 바이트(2진 파일) 인터프리터(java)
	 * 		두 번의 검증을 거친다(보안이 뛰어나다)
	 * 
	 *  = 객체 지향적 프로그램(재사용 편리한 상태) => 유지보수(대규모 프로그램)
	 *  
	 *  재사용 = 추가, 수정, 데이터 보호(캡슐화 ) --> 5장에서
	 *  
	 *         --------오버로딩, 오버라이딩
	 *         
	 ...
	 *     6) 객체 지향 프로그램
	 *     7) 프로그램이 비정상 종료를 방지 / 오류를 처리 = 예외 처리
	 *     ============================================== 자바기본
	 *     8) 자바에서 지원하는 라이브러리
	 *     
	 *  3. 자바의 구성
	 *     package  : 한번만 사용 => 위치
	 *     import   : 기존에 만들어진 클래스를 불러온다.(여러개 사용가능)
	 *     --------------------------------------------------
	 *     class ClassName{
	 *         ---------------------
	 *         변수 설정
	 ...
	 *    
	 *     *** 변수
	 *     => 한 개의 데이터를 저장하는 메모리 공간 (메모리주소에 이름을 부여하는 것)
	 * 	   => 사용법
	 *        데이터형 변수형 = 값;
	 *        ------
	 ...
	 *       2byte => short, short a = 10; // C 언어와 호환
	 *       4byte => int,  int a = 10;  // 평상시에 사용하는 모든 정수(default), 메모리 단위가 4byte 최적화, 속도
	 *       System.out.println(100);
	 *       8byte => long, long a = 10;
	 *     실수 저장 방법
	 *     4byte float  ==> 소수점 6자리 
	 *     8byte double (default) 소수점 15자리
	 *     10.5F(float) 10.5D(생략이 가능)
	 *     문자 저장 방법
	 *     2byte => char
	 *       => 1byte(싱글바이트 => ASC) / 2byte(멀티바이트 => UniCode)
	 *       *** 한글이나 중국, 독일, 일본.. 한글당 2byte
	 *       1 byte = 8 bit = 256문자
	 *       2 byte = 16 bit = 65535 문자 사용이 가능 
	 *       UTF-8
	 *       각 문자 마다 번호가 있다.
	 *     논리 저장 방법
	 *        true / false 만 저장 => 1byte => boolean
	 *     자동 지정 (jdk 10 이상)
	 *        var a =10;  int
	 *        var b = 10.3;  double
	 *        var c = 'A'; char
	 *        
	 *     => 저장이 가능한 데이터
	 *     1byte => -128 ~ 127   => byte a = 128; error
...
//		System.out.println(a);//10
		...
		// 127을 byte_value 라는 공간에 첨부해라
		short short_value = 32767; //-32768 ~ 32767
		//*****
		int int_value = 2140000000; // -2140000000 ~ 2140000000
		//*****
		long long_value = 999999999999999999L;  
		//int/long 의 구분은 l/L로 구분한다.
		char char_value = '홍';
		float float_value = 10.5f;
		//*****
		double double_value = 10.5;//d는 생략가능
		//*****
		...
		final int SCORE = 70;
		//SCORE = 100; // error 상수 변경불가 대문자로 선언
