 ...
 *       -----------------------
 *        메소드 (멤버메소드) => static 이 없는 메소드
 *       -----------------------
 *        생성자 : 멤버 변수의 초기화, 객체 생성시에 호출되는 메소드
 *       -----------------------
 *       
 *    }
 *    
 ...
 *       예) 웹 사이트
 *          회원 : 회원가입, 회원탈퇴, 회원 수정, 로그인,로그아웃,아이디 찾기
 *          --- 부품
 ...
 *         사람
 *         이름
 *         성별         => class 사람{String name,sex,jumin, int age}; ==> 메모리에 저장 (인스턴스)
 *         주민번호
 *         나이
 *         먹는다
 *         달린다
 ...
 *         }
 *         
 *         클래스로부터 객체 생성하기 위해서는 변수와 메소드가 정의
 *                                    ----------
 *         클래스를 저장하면 인스턴스(객체)
 *         A a = new A();
 *         - -객체명
 ...
 *              a = new A();
 *              
 *              A a = (return int)malloc(sizeof(A))
 *                                ------------------ 연산자로 승격(new)
 *              class A{
 *                int a; // 주소는 4byte
 *                int b; // 주소는 4byte
