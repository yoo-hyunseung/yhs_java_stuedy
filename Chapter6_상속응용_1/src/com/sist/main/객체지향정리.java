 ...
 *          구현부가 있어야 저장이된다.
 *     }
 *     ==> 메소드 선언만 된 경우에는 클래스를 저장하지 못한다 (미완성한 클래스)
 *         (추상 클래스 , 인터페이스) => 요구사항 분석
 ...
 *            } 
 *    ------------------
 *    Heap : 동적 메모리 할당 (new)  ==> 객체변수 , 배열(참조변수형)
 *           => 관리 (프로그래머) => GC(자동메모리 회수)
 *           => 객체 = null
 *    A.java ==> A.class ==>
 *          javac       java  
 *    class A{ => 인터프리터 => 한줄씩 번역하는거
 *       ---
 *       ---
 *       ---
 *       ...
 ...
 *         Getter / Setter 는 메소드 (변수의 기능 : 읽기 / 쓰기)
 *       = 메소드만 모아서 관리 : 액션 클래스  기능만 가진 것
 ...
 *       생성자 : public 
 *    4. 객체지향의 특성 
 *       1) 캡슐화 => 데이터를 은닉화 한후에 메소드를 통해서 접근이 가능 
 *                   ---------- private => getter / setter
 *       2) 재사용 
 *          => 상속 (is-a) : 기존의 기능을 변경해서 사용
 *          class A{
 *           }
 *           class B extends A{} ==> 단일 상속
 *          => 포함 (has-a) : 기존의 기능을 있는 그대로 사용
 *           class A{}
 *           class B{
 *              A a;
 *              String name; ... 변경하지 않고 그대로 사용
 *           }
 *          
 *       3) 수정, 추가 => 다형성
 *          오버라이딩 , 오버로딩   
