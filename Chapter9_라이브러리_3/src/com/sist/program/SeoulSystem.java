...
public class SeoulSystem {
	...
	private int curpage; // 프로그램 종료시 까지 유지
	private int totalpage;
	static 
	{
		// data를 읽어서 메모리에 저장 초기화 -> datas 의 초기화
		try {
			StringBuffer sb = new StringBuffer();
			int i = 0;
			FileReader fr = new FileReader("/Users/yuhyeonseung/Desktop/sist/javaDev/seoul_location.txt");
			while((i = fr.read()) != -1) {
...
//				System.out.println(s);
	...
	public SeoulLocationVO[] seoulList(int page) {
		SeoulLocationVO[] data = new SeoulLocationVO[10];// 10개당 1page
		int j = 0; // 10 page씩 나눠주는 변수
	...
	public int seoulTotalPage() {
		return (int)(Math.ceil(datas.length/10.));
//		             --------------------------- 27.3 -> 28  ceil
	...
	public void process() {
		...
		while(true) {
			...
			if(menu == 9) {
				...
				Scanner scan = new Scanner(System.in);
				System.out.print("상세 볼 명소번호 입력(1~ 273) : ");
				int no = scan.nextInt();
				System.out.println("번호 : "+datas[no-1].getNo());
				System.out.println("이름 : "+datas[no-1].getName());
				System.out.println("내용 : "+datas[no-1].getContent());
