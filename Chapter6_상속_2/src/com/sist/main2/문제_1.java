...
/*
 *    생성자 
 *    1) 모든 클래스의 이름은 클래스명과 동일하다.
 *    2) 생성자는 객체를 생성하기 위한 것이다.(X) -> new 
 *    3) 클래스에는 생성자를 바드시 하나 이상 있어야 된다.(X)
 *       => 생성자는 없어도 상관이 없다.
 ...
 *           //public int display(int n){}  
         ...
         2) 객체 자신을 가리키는 참조변수 (static Object this) = > new 
            ...
            class A{
                ...
                pubilc stati void aaa(){}
            }
         3) 클래스 내에서는 어디서든 사용이 가능하다 (X)  static메소드에서는 사용 불가능
 ...
 *              
		...
		c. 지역변수에도 접근 제어자를 사용할 수 있다. // 
	       ------ 
	       지역변수는 static , 접근 지정어 , abstract 는 사용이 불가능
...
public class 문제_1 {
	public static void main(String[] args) {
		A a = new A(); // b가 먼저 출력된다 => 클래스변수가 먼저 초기화된다
	}
}
 ...
 *    => kind, num
 * 정적변수(공유변수) 공동으로 사용되는 변수 -> 메모리가 1개만 생성
 *    => width, height
 * ------------------------프로그램 종료시 까지 메모리 유지 (자동초기화)
 * 지역변수 : 메소드에서 선언되는 변수 (지역변수 , 매개변수) 
 *         => 메소드 안에서 사용 ({}를 벗어나면 사라진다.)
 *         => 반드시 초기화후에 사용
 *         => k , n
 */
