 ...
 *      import 외부 라이브러리 불러오기 ==> 여러번 사용가능 
 *      public class ClassName{
 *          -----------------------
 *          멤버변수
 *           = 인스턴스 변수 => new 를 사용할 때 마다 메모리가 따라 생성
 *           = 정적변수 (공유변수) => 메모리가 한개만 생성
 *          -----------------------
 *          생성자 : 메모리 할당시 호출 되는 메소드
 *                  = 멤버변수에 대한 초기화
 *          -----------------------
 *          메소드 : 특정 작업을 수행하는 명령문의 집 ******
 *          -----------------------
 *          main() : 프로그램의 시작점
 *          -----------------------
 *      }
 ...
 *                         *** 3개 이상이면 => 배열, 클래스
 *              형식)
 *               두개 정수를 보내고 => 더한 값을 받는다.
 *               int add(int a, int b){//선언부 	
 *               --- 클수도 있다. double , float , long  => int 권
 *                   // 구현부
 *                   int c = a+b;
 *                   return c;
 *                         --- 동일해야 한다.
 ...
 *                   => void 일때는 생략이 가능 => 생략을 하면 컴파일러가 자동으로 return을 추가한다.
 *                   
 *                   
 *               }
 *               ** 변수(20%) / 메소드(80%)
 ...
 *   	return true;
 *   }
 *   boolean bCheck = isLogin("sdff", "12314");
 *   -------
 *   
 *   
 ...
 *    gugudan();
 *    
 *   => 메소드를 호출하면 메소드 전체를 수행하고 => 호출된 위치로 복귀 한다.
 *   
 *   main() {
 *       1
 *       2
 *       3 =========== 수행
 *       gugudan(); ======== 7 8 9 10 11 수행
 *       4 =========== 수행시작
 *       gugudan(); ======== 7 8 9 10 11 수행
 *       5
 *       6
