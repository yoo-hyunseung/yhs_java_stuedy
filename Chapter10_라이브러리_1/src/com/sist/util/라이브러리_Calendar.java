 ...
 *           저장 => 일반 기본형 , 사용자 정의 클래스 => 기본적으로 설정 Object
 *           형변환 
 *           => 주요기능 
 *              clone() : 복제 (새로운 메모리 생성) =>디자인패턴 (GOLF 23)
 ...
 *                    -> 웹, 윈도우, 빅데이터(AI)
 *                    -> equals(), trim(), substing(), length(),
 *                       valueOf(), contains(), startsWith(),
 *                       split(), replace(), replaceAll()
 *                       
 *     java.util : 사용빈도가 많다.
 *     ---------
 *         = StringTokenizer : 문자열 분리 (쉽게 분리)
 *           = countToken(); => 분리된 단어의 토큰
...
public class 라이브러리_Calendar {
	...
	public static void main(String[] args) {
		...
		// 사용자로 부터 년도/월 => 예약/체크인,체크아웃 => 달력
		Scanner scan = new Scanner(System.in);
		System.out.print("년도 월 입력:(2023 4)");
		int year = scan.nextInt();
		int month = scan.nextInt();
		
		// 1일 자의 요일을 가지고 온다.(4 -> 4/1, 요일)
		Calendar cal = Calendar.getInstance();
		// 추상 클래스는 미완성된 클래스 (메소드가 구현이 안된 상태)
		// 구현할 클래스를 이용한다. , 싱글턴 (newInstance(), getInstance())
		/*
		 *   static => 클래스명.메소드명();
		 *   Math.random()
		 *   클래스명 객체명 = new 생성자(); => 두개가 다른거는 -> 추상클래스 or 상속 or 인터페이스
		 *   -----             ----        클래스,생성자명
		 *   클래스명 객체명 = 메소드(); -> 추상클래스
		 */
		//
		...
		cal.set(Calendar.DATE, 1); // 1일자 요일 확인을 위함
